...
export default function CommunitySection() {
    const points = [
        <>
          Learn <span className="font-semibold text-[#0F766E]">Machine Learning</span>,
          <span className="font-semibold text-[#0F766E]"> Artificial Intelligence</span>,
  ...
  return (
    <section className="relative overflow-hidden bg-white font-[Inter,Helvetica_Neue,Arial,sans-serif]">
      <div className="mx-auto grid min-h-[760px] max-w-[1800px] grid-cols-1 items-center lg:grid-cols-[52%_48%]">
        <div className="px-8 py-20 sm:px-16 lg:pl-[160px] lg:pr-20">
          <h2 className="max-w-[720px] text-[48px] font-black italic leading-[0.92] tracking-[-0.04em] text-zinc-950 ">
            Discover more than
            <br />
            a classroom — join a
            <br />
          ...
          <ul className="mt-9 max-w-[700px] space-y-6 text-[22px] leading-[1.35] text-slate-700">
            {points.map((point, index) => (
              <li key={index} className="grid grid-cols-[10px_1fr] gap-6">
                <span className="mt-3 h-1.5 w-1.5 rounded-full bg-slate-700" />
                <span>{point}</span>
              </li>
            ))}
          </ul>
